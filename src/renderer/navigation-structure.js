(() => {
  ...
  function replaceLegacyText(root) {
    ...
    for (const node of nodes) {
      ...
      for (const [legacy, replacement] of LEGACY_COPY) value = value.replaceAll(legacy, replacement);
  ...
  function bindCopySync() {
    document.querySelectorAll('.nav-item').forEach((button) => {
      ...
      button.addEventListener('click', () => {
        window.setTimeout(() => syncToolPresentation(button.dataset.tool), 0);
      });
    });
  }
  ...
  function installCopyObserver() {
    ...
    copyObserver = new MutationObserver((records) => {
      ...
      for (const record of records) {
        ...
        for (const node of record.addedNodes || []) replaceLegacyText(node);
      }
      applyStaticCopy();
      syncToolPresentation();
    });
    copyObserver.observe(document.body, { subtree: true, childList: true, characterData: true });
  }

  function initialize(attempt = 0) {
    installStyles();
    if (!restructureNav()) {
      if (attempt < 30) window.setTimeout(() => initialize(attempt + 1), 50);
      return;
    }
    installCopyObserver();
  }
