...
assert.match(service, /if \(modelId === 'topaz'\) return \{ image_url: asset\.image_url, width, height \}/);
assert.match(service, /const width = Math\.round\(asset\.width \* scale\)/);
assert.match(service, /const height = Math\.round\(asset\.height \* scale\)/);
assert.match(service, /failure_reason/);
assert.doesNotMatch(service, /model: 'Standard V2'/);
assert.doesNotMatch(service, /upscale_factor: scale/);
assert.doesNotMatch(service, /face_enhancement_strength/);
