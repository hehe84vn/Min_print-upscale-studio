...
assert.doesNotMatch(service, /model: 'Reimagine'/);
assert.match(service, /regenerationPreset/);
assert.match(service, /aiStrength: 0\.15/);
assert.match(service, /resemblance: 2\.3/);
assert.match(service, /targetLongEdge/);
assert.match(service, /maxDimension: 22000/);
assert.match(service, /maxScale: 32/);
assert.match(service, /withMetadata\(\{ density: dpi \}\)/);
assert.match(service, /jpeg\(\{ quality: 96/);
assert.match(service, /tiff\(\{ compression: 'lzw'/);
assert.match(service, /webp\(\{ lossless: true/);
assert.match(service, /Krea trả về sai kích thước/);
assert.match(service, /resolutionOk/);
...
assert.match(ui, /long:4000/);
assert.match(ui, /long:8000/);
assert.match(ui, /long:12000/);
assert.match(ui, /long:16000/);
assert.match(ui, /long:22000/);
assert.match(ui, /regenerationMode:selectedMode\(\)/);
assert.match(ui, /outputFormat:\$id\('formatSelect'\)\.value/);
assert.match(ui, /dpi:Number\(\$id\('dpiSelect'\)\.value\)/);
assert.match(ui, /Faithful · khóa bảo toàn/);
assert.match(ui, /có thể thay đổi chữ, logo/);
...
console.log('Cloud provider smoke test passed for Krea model payloads, sizing, print export and KiraAI wiring.');