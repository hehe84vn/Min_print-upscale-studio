...
assert.match(service, /upscaling_activated: sizing\.scale > 1/);
assert.match(service, /image_scaling_factor: sizing\.scale/);
assert.match(service, /targetLongEdge/);
assert.match(service, /maxDimension: 22000/);
assert.match(service, /maxScale: 32/);
assert.match(service, /outputWidth/);
assert.match(service, /outputHeight/);
assert.match(service, /failure_reason/);
assert.doesNotMatch(service, /upscale_factor: scale/);
...
assert.match(ui, /K là số pixel của cạnh dài/);
assert.match(ui, /In 300 DPI/);
...
console.log('Cloud provider smoke test passed for Krea sizing presets and KiraAI beta wiring.');