...
const [service, ipc, bootstrap, preload, ui, loader, deliveryRule] = await Promise.all([
  ...
  fs.readFile(preloadPath, 'utf8'), fs.readFile(uiPath, 'utf8'), fs.readFile(loaderPath, 'utf8'),
...
assert.match(ui, /kreaBetaEndpointInput/);
assert.match(ui, /kreaBetaEnabled/);
assert.match(ui, /kreaBetaRunMode/);
assert.match(ui, /kreaBetaEnhanceMode/);
...
console.log('Krea beta smoke test passed with module toggle, local/Krea A/B comparison and delivery status rule.');